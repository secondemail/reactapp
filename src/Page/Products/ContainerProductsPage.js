...
import { categoryProducts } from '../../redux/actions/productsAction'
import { useParams } from 'react-router-dom';

const ContainerProductsPage = () => {
    ...
    useEffect(() => {
        const get = async () => {
            setLoading(true);
            await dispatch(categoryProducts(id));
            setLoading(false);
        };
        get();
    }, []);
    const res = useSelector(state => state.allproducts.categoryProducts);
    return (
